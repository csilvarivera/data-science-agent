...
from .prompts import return_instructions_planner
from .sub_agents import bqml_agent, python_ml_agent

from .sub_agents.bigquery.tools import (
    get_database_settings as get_bq_database_settings,
)
from .tools import call_bigquery_agent, call_bqml_agent, call_python_ml_agent
...
def get_root_agent() -> LlmAgent:
    ...
    for dataset in _dataset_config["datasets"]:
        if dataset["type"] == "bigquery":
            ...
            tools.append(call_bqml_agent)
            tools.append(call_python_ml_agent)
            sub_agents.append(bqml_agent)
            sub_agents.append(python_ml_agent)

    agent = LlmAgent(
        model="gemini-3.1-pro-preview",
        name="data_science_planner_agent",
        instruction=return_instructions_planner()
        + get_dataset_definitions_for_instructions(),
        global_instruction=(
            f"""
            You are a Data Science Planner Agent.
